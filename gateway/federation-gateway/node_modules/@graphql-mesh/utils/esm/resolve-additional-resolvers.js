                ...
                [additionalResolver.targetFieldName]: {
                    ...
                    resolve: (payload, _, ctx, info) => {
                        ...
                        if (additionalResolver.sourceName) {
                            ...
                            if (!stitchingInfo) {
                                throw new Error(`Stitching Information object not found in the resolve information, contact maintainers!`);
                            }
                            const returnTypeName = getNamedType(info.returnType).name;
                            const mergedTypeInfo = stitchingInfo?.mergedTypes?.[returnTypeName];
                            if (!mergedTypeInfo) {
                                throw new Error(`This "${returnTypeName}" type is not a merged type, disable typeMerging in the config!`);
                            }
                            const subschema = Array.from(stitchingInfo.subschemaMap?.values() || []).find(s => s.name === additionalResolver.sourceName);
                            if (!subschema) {
                            ...
                            if (!resolver) {
                                throw new Error(`The type "${returnTypeName}" is not resolvable from the source "${additionalResolver.sourceName}", check your typeMerging configuration!`);
                            }
                            const selectionSet = info.fieldNodes[0].selectionSet;
                            return handleMaybePromise(() => resolver(payload, ctx, info, subschema, selectionSet, undefined, info.returnType), handlePayload);
                        }
