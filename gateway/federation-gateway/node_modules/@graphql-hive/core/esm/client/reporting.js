...
export function createReporting(pluginOptions) {
    var _a, _b, _c, _d;
    ...
    const logger = createHiveLogger((_b = (_a = pluginOptions.agent) === null || _a === void 0 ? void 0 : _a.logger) !== null && _b !== void 0 ? _b : console, '[hive][reporting]');
    logIf(typeof reportingOptions.author !== 'string' || reportingOptions.author.length === 0, '[hive][reporting] author is missing', logger.error);
    logIf(typeof reportingOptions.commit !== 'string' || reportingOptions.commit.length === 0, '[hive][reporting] commit is missing', logger.error);
    ...
    return {
        async report({ schema }) {
            ...
            try {
                const response = await http.post(endpoint, JSON.stringify({
                    query,
...
async function extractFederationServiceSDL(schema) {
    ...
    if (resolved.sdl.includes('_service')) {
        ...
        return print(visit(parse(resolved.sdl), {
...
async function printToSDL(schema) {
    return stripIgnoredCharacters(isFederatedSchema(schema)
