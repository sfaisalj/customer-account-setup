...
function createAgent(pluginOptions, { data, body, headers = () => ({}), }) {
    ...
    async function send(sendOptions) {
        ...
        const response = await http_client_js_1.http
            .post(options.endpoint, buffer, {
            headers: Object.assign({ accept: 'application/json', 'content-type': 'application/json', Authorization: `Bearer ${options.token}`, 'User-Agent': `${options.name}/${version_js_1.version}` }, headers()),
