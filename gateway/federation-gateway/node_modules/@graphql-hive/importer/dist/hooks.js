...
import Ae from 'fs';
import { d as debug, t as transpileTypeScriptFile } from './transpile-DfP8LJuu.js';
...
const resolve = async (specifier, context, nextResolve) => {
  ...
  if (!specifier.startsWith(".") && packedDepsPath) {
    try {
      ...
      const resolved = resolveFilename(p$1.join(packedDepsPath, specifier));
      ...
      for (const nodePath of nodePaths.split(",").map((v) => v.trim())) {
        try {
          ...
          const adjustedSpecifier = fixSpecifier(
            resolveFilename(p$1.join(nodePath, specifier)),
            context
          );
...
const load = async (url, context, nextLoad) => {
  if (p$1.sep === "\\" && !url.startsWith("file:") && url[1] === ":") {
