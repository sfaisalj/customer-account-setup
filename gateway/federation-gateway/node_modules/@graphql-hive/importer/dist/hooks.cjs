...
var p$1 = require('node:path');
var node_url = require('node:url');
var se = require('node:fs');
var Ae = require('fs');
var transpile = require('./transpile-Brs8HMxu.cjs');
require('node:fs/promises');

function _interopDefault (e) { return e && e.__esModule ? e : { default: e }; }

var module__default = /*#__PURE__*/_interopDefault(module$1);
var p__default = /*#__PURE__*/_interopDefault(p$1);
...
const initialize = (data = {}) => {
  if (data.packedDepsPath) {
    ...
    transpile.debug(`Packed dependencies available at "${packedDepsPath}"`);
  }
  const tsconfig = he(
    void 0,
    data.tsconfigSearchPath || process.env["HIVE_IMPORTER_TSCONFIG_SEARCH_PATH"] || "tsconfig.json"
  );
  if (tsconfig) {
    transpile.debug(`tsconfig found at "${tsconfig.path}"`);
    pathsMatcher = Pe(tsconfig);
  }
};
function fixSpecifier(specifier, context) {
  if (p__default.default.sep === "\\") {
    if (context.parentURL != null && context.parentURL[1] === ":") {
      context.parentURL = node_url.pathToFileURL(
    ...
    if (specifier.startsWith("file://")) {
      specifier = node_url.fileURLToPath(specifier);
    }
    if (!specifier.startsWith(".") && !specifier.startsWith("file:") && specifier[1] === ":") {
      specifier = node_url.pathToFileURL(specifier).toString();
    }
  }
  if (specifier.startsWith("node_modules/") || specifier.startsWith("node_modules\\")) {
    specifier = specifier.replace("node_modules/", "").replace("node_modules\\", "").replace(/\\/g, "/");
  }
  return specifier;
}
const resolve = async (specifier, context, nextResolve) => {
  ...
  if (!specifier.startsWith(".") && packedDepsPath) {
    try {
      ...
      const resolved = resolveFilename(p__default.default.join(packedDepsPath, specifier));
      transpile.debug(`Possible packed dependency "${specifier}" to "${resolved}"`);
      return await nextResolve(fixSpecifier(resolved, context), context);
    } catch {
    }
  }
  try {
    transpile.debug(`Trying default resolve for "${specifier}"`);
    return await nextResolve(specifier, context);
  } catch (e) {
    transpile.debug(`Default resolve for "${specifier}" failed, trying alternatives`);
    const nodePaths = process.env["NODE_PATH"] || "";
    if (nodePaths) {
      transpile.debug(`NODE_PATH set to "${nodePaths}", trying its paths`);
      for (const nodePath of nodePaths.split(",").map((v) => v.trim())) {
        try {
          transpile.debug(`Trying "${specifier}" under "${nodePath}"`);
          const adjustedSpecifier = fixSpecifier(
            resolveFilename(p__default.default.join(nodePath, specifier)),
            context
          );
      ...
      transpile.debug(`Resolving with NODE_PATH paths failed, trying other alternatives`);
    }
    try {
      const specifierWithoutJs = specifier.endsWith(".js") ? specifier.slice(0, -3) : specifier;
      const specifierWithTs = specifierWithoutJs + ".ts";
      const adjustedSpecifier = fixSpecifier(specifierWithTs, context);
      transpile.debug(`Trying "${adjustedSpecifier}"`);
      return await nextResolve(adjustedSpecifier, context);
    } catch (e2) {
      try {
        transpile.debug(`Trying fixed "${specifier}"`);
        const adjustedSpecifier = fixSpecifier(
        ...
        try {
          const specifierWithoutJs = specifier.endsWith(".js") ? specifier.slice(0, -3) : specifier;
          transpile.debug(`Trying "${specifierWithoutJs + ".ts"}"`);
          const adjustedSpecifier = fixSpecifier(
            resolveFilename(
              specifierWithoutJs + ".ts"
              // TODO: .mts or .cts?
            ),
            context
          );
          ...
          if (pathsMatcher) {
            for (const possiblePath of pathsMatcher(specifier)) {
              try {
                transpile.debug(`Trying "${possiblePath}"`);
                const adjustedSpecifier = fixSpecifier(
                ...
                try {
                  const possiblePathWithoutJs = possiblePath.endsWith(".js") ? possiblePath.slice(0, -3) : possiblePath;
                  transpile.debug(`Trying "${possiblePathWithoutJs + ".ts"}"`);
                  const adjustedSpecifier = fixSpecifier(
                    resolveFilename(
                      possiblePathWithoutJs + ".ts"
                      // TODO: .mts or .cts?
                    ),
                    context
                  );
...
const load = async (url, context, nextLoad) => {
  if (p__default.default.sep === "\\" && !url.startsWith("file:") && url[1] === ":") {
    transpile.debug(`Fixing Windows path at "${url}"`);
    url = `file:///${url.replace(/\\/g, "/")}`;
  }
  if (/\.(m|c)?ts$/.test(url)) {
    const { format, source } = await transpile.transpileTypeScriptFile(url);
    return {
      format,
      source,
      shortCircuit: true
